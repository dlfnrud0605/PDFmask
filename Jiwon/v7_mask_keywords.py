...
NOUNISH_INCLUDE = {"SL", "SN"}   # 영문, 숫자까지 포함. 순수 한글 명사만 원하면 set()로.

# 표시/가림막 스타일
HIGHLIGHT_COLOR = (1, 0, 0)   # 빨강
LINE_WIDTH      = 1.8
STROKE_ALPHA    = 1.0
FILL_ALPHA      = 0.0         # highlight일 때 채움 없음
FILL_COLOR      = None
...
kiwi = Kiwi(num_workers=-1)  # Win에서 안정
...
def collect_line_chars(line):
    # 문자 bbox가 없는 span은 건너뜀(폴백 균등분할은 높이 과장 문제 유발)
    out = []
...
def run(pdf_in, pdf_out):
    ...
    for pno in range(len(src)):
        ...
        if MODE == "redact":
            # 1) 흰색으로 리덕션 적용
            for r in rects:
                annot = mpage.add_redact_annot(r, fill=(1, 1, 1))  # 내부 흰색
                # 적용 전 미리보기용 테두리(선택)
                try:
            ...
            for r in rects:
                mpage.draw_rect(
                    ...
                    color=STROKE_COLOR,
                    width=STROKE_WIDTH,
                    fill=None,          # 내부 채움 없음(이미 흰색으로 칠해짐)
                    overlay=True
                )
            else:  # highlight
                for r in rects:
                    mpage.draw_rect(
                        ...
                        fill=FILL_COLOR, fill_opacity=FILL_ALPHA, overlay=True
                    )

        print(f"  - {pno+1}/{len(src)} 페이지 완료 (박스 {len(rects)})")
